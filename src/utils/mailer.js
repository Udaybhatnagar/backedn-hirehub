...
// ── Startup guard: warn immediately if email creds are missing ───────────────
if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
  console.error(
    "[Mailer] ❌ EMAIL_USER or EMAIL_PASS env vars are not set. " +
    "OTP emails will fail. Set them in your Render environment settings."
  )
}
...
const transporter = nodemailer.createTransport({
  host: "smtp.gmail.com",
  port: 587,
  secure: false,       // STARTTLS (not SSL — avoids IPv6 issues on Render)
  requireTLS: true,
